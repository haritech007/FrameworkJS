...
window[NS] = (function(window) {
	...
	function getRouteAndData(path) {
		var route = plugin.appRoutes.filter(function(item) {
	...
	function loadTemplate(template) {
		var appRoot = document.getElementById(plugin.appRoot);
	...
	function publishRouteData(data) {
		plugin.routeData = data;
	}

	function getRouteData() {
		return plugin.routeData;
	}
	...
	function init(appRoot, appRoutes) {
		plugin.appRoutes = appRoutes;
		plugin.appRoot = appRoot;
		bindEvents();
	}
	...
	function setContext(contextObj) {
		if(Object.keys(contextObj).length) {
			for(keys in contextObj) {
				plugin.context[keys] = contextObj[keys]; 
			}
		}
	}
